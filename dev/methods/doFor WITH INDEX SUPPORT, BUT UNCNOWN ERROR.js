...
export default function doFor(str) {
    ...
    elements.forEach(element => {
        ...
        evalArray.forEach((item, index) => {
            ...
            this.log('Iteration', `Key is ${key}, Lndex is ${idx}, Iterated arr ${arr}, is `, evalArray);
        ...
        // console.log('evalArray: ', evalArray);
            ...
            while (walker.nextNode()) {
                ...
                if (node.nodeType === 3 && node.textContent.includes('[')) {
                    ...
                    // console.log(`[${this.tagName}] node.textContent ---`, node.textContent);
                    // console.log('key: ', key);
                    // console.log('idx: ', idx);
                    // console.log('item: ', item);
                    // console.log('index: ', index);
                    node.textContent = replaceBrackets(node.textContent, key, idx, item, index, component);
                }
...
// console.log(`[${component}] text ---`, text);
    return text.replace(/\[([^\[\]]+(\[[^\[\]]*\])?[^\[\]]*)\]/g, (match, expr) => {
        try {
            ...
            console.warn('Expression error in bracket:', expr, err);
            return match;
        }
    });
}
