...
com({
  ...
  tpl() {
    return html`
      <div class="jet-form" :class="position class form_style">
        <h3>{{el.title}}</h3>
        <input j-model="obj.title" />
        <!-- ${this.el.fields
          .map((e) => {
            if (e.type === 'push') {
              return `<jet-field-push p:obj="obj" #key="${e.key}" #ops="${btoa(
  ...
  created() {
    //get data objects
    // this.obj = this.proxy(JSON.parse(atob(this.slots.obj)));
    // this.el = this.proxy(JSON.parse(atob(this.slots.el)));
  ...
  mount() {
    ...
    // console.log(`%c [${this.tagName}]el-------- `, 'color: #f55308', this.slots);
    console.log(`%c [${this.tagName}obj-------- `, 'color: #f55308', this.obj);
...
function renderFields(e, obj = 'obj') {
  ...
  let html = `<div class="control-field">`;
  html += `<label>${e.label}:</label>`;
  html += `<jet-field-${e.type} p:obj="${obj}" #key="${e.key}" #ops="${btoa(
    JSON.stringify(e.ops)
  )}"></jet-field-${e.type}>`;
  html += `</div>`;
  return html;
